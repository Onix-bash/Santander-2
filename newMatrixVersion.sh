...
source_to_check_changes="origin/main"
...
checkFolders() {
  ...
  git_diff=$(git diff --name-only $source_to_check_changes --no-index)
  ...
  compare_modules() {
    all_modules=( $(cd src/; ls -1p | grep / | sed 's|/$||') )
      original_dir=$(pwd)
      for module_name in "${all_modules[@]}"; do
          if jq -e --arg module_name "$module_name" 'has($module_name)' <<< "$json_output" >/dev/null; then
             all_folders=$(jq -r --arg module_name "$module_name" '.[$module_name][]' <<< "$json_output")
             cd "$original_dir" || exit 1
                     for folder_name in $all_folders; do
                       echo "$folder_name"
                       echo "$module_name"
                       # Go to folder with CalculationMatrixVersion.json
                       cd "src/$module_name/data/$folder_name" || exit 1
    #                   set_input_version
                     done
          fi
      done
