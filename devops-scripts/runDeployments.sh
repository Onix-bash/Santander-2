...
if [ "$SKIP_ORG_INTERACTION" = "true" ]; then
  ...
  command="sf project deploy start --manifest sourcePackage.xml --post-destructive-changes "src/destructiveChanges/destructiveChanges.xml" -l NoTestRun --ignore-conflicts --ignore-warnings --async --json"
  echo "$command"
#  deploy_command_result=$(sf project deploy start --manifest sourcePackage.xml --post-destructive-changes "src/destructiveChanges/destructiveChanges.xml" -l NoTestRun --ignore-conflicts --ignore-warnings --async --json)
