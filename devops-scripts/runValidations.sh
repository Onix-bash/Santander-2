...
if [ "$SKIP_ORG_INTERACTION" = "true" ]; then
  ...
  validate_command_result=$(sf project deploy start --manifest sourcePackage.xml --post-destructive-changes "src/destructiveChanges/destructiveChanges.xml" --ignore-warnings --ignore-conflicts --verbose $unit_tests_params --dry-run --async --json)
